...
export function Footer({ t, setViewMode, footerText, copyrightText }: FooterProps) {
  ...
  useEffect(() => {
    fetch('/api/design')
      .then(r => r.json())
      .then(d => setBranding(d?.branding || d?.data?.branding || {}))
      .catch(() => {});
  }, []);

  const social = branding?.social || {};
  const showLogo = branding?.showFooterLogo !== false;
  const hasSocials = Object.values(social).some(url => url);

  return (
    <footer className="mt-20 bg-[#161311]/95 backdrop-blur-xl px-4 md:px-8 py-12 shadow-2xl">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
        <div className="space-y-4">
          {showLogo && <img src={branding?.logo || "/LOGO-BEAT.png"} alt="HEAVIEXO BEATS" className="h-10 w-auto object-contain" />}
      ...
      <div className="max-w-7xl mx-auto pt-6 border-t border-white/10 text-[11px] text-[#9E938B] font-medium text-center">
        <p>{branding?.copyright || copyrightText}</p>
      </div>
    </footer>
  );
}