...
export default function AdminSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    console.log('SAUVEGARDE payload:', payload);
    ...
    console.log('SAUVEGARDE réponse:', data);

    if (data.success) {
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
      const refresh = await fetch('/api/design');
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBranding((prev: any) => ({ ...prev, logo: blob.url }));
    } catch (err) { alert('Erreur upload logo'); }
  ...
  const handleFaviconUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setBranding((prev: any) => ({ ...prev, favicon: blob.url }));
    } catch (err) { alert('Erreur upload favicon'); }
      ...
      <div className="bg-[#171513] border border-[#26221f] rounded-2xl p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {PRESETS.filter(p => p.id !== 'CUSTOM').map(preset => (
            <button type="button" key={preset.id} onClick={() => handlePresetChange(preset.id)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activePreset === preset.id ? 'bg-[#ff6b35] text-white' : 'bg-[#201d1a] text-gray-400 hover:text-white'}`} style={activePreset === preset.id ? {} : { borderLeft: `3px solid ${preset.primary}` }}>{preset.name}</button>
          ))}
          <button type="button" onClick={() => handlePresetChange('CUSTOM')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activePreset === 'CUSTOM' ? 'bg-[#ff6b35] text-white' : 'bg-[#201d1a] text-gray-400 hover:text-white'}`}>Custom</button>
        </div>
        {activePreset === 'CUSTOM' && (
